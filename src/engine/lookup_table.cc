...
{
  std::fill(std::begin(hand_rank_), std::end(hand_rank_), 0);
  ...
  std::fill(std::begin(card_ids), std::end(card_ids), 0);
  ...
  for (num_id = 0; card_ids[num_id] || num_id == 0; num_id++) {
  ...
  for (num_id = 0; card_ids[num_id] || num_id == 0; num_id++) {
    // start at 1 so I have a zero catching entry (just in case)
    for (card = 1; card < 53; card++) {
      id = MakeID(card_ids[num_id], card);
      ...
      int max_hand_rank = num_id * 53 + card + 53;	// find where to put it
      hand_rank_[max_hand_rank] = slot_id; // and save the pointer to the next card or the handrank
    }
    ...
    if (numcards == 6 || numcards == 7) {
      ...
      hand_rank_[num_id * 53 + 53] = DoEval(card_ids[num_id]);
    }
...
int LookupTable::DoEval(int64_t IDin)
{
...
{
  FILE *fin = fopen(kOutputPath, "rb");
	if (!fin) {
	  return false;
	}
	fread(hand_rank_, sizeof(hand_rank_), 1, fin);
	fclose(fin);
	return true;
}

int LookupTable::LookupHand(int* pCards)
{
    int p = hand_rank_[53 + *pCards++];
    p = hand_rank_[p + *pCards++];
    p = hand_rank_[p + *pCards++];
    p = hand_rank_[p + *pCards++];
    p = hand_rank_[p + *pCards++];
    p = hand_rank_[p + *pCards++];
	return hand_rank_[p + *pCards++];
}

int64_t LookupTable::MakeID(int64_t IDin, int newcard)
{
...
void LookupTable::SaveToFile()
{
  FILE *fout = fopen(kOutputPath, "wb");
  if (fout) {
    fwrite(hand_rank_, sizeof(hand_rank_), 1, fout);
    fclose(fout);
  }
}
